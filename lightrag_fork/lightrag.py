...
from lightrag_fork.exceptions import PipelineCancelledException
...
from lightrag_fork.kg.shared_storage import (
    get_namespace_data,
    get_data_init_lock,
...
from lightrag_fork.kg.lock_backend import LockLostError

from lightrag_fork.base import (
    BaseGraphStorage,
    BaseKVStorage,
    BaseVectorStorage,
    DocProcessingStatus,
...
from lightrag_fork.namespace import NameSpace
from lightrag_fork.operate import (
...
from lightrag_fork.utils import (
    Tokenizer,
    TiktokenTokenizer,
    EmbeddingFunc,
...
from lightrag_fork.types import KnowledgeGraph
...
class LightRAG:
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            from lightrag_fork.kg.shared_storage import initialize_pipeline_status
    ...
    def _get_storage_class(self, storage_name: str) -> Callable[..., Any]:
        ...
        if storage_name == "JsonKVStorage":
            from lightrag_fork.kg.json_kv_impl import JsonKVStorage

            return JsonKVStorage
        elif storage_name == "NanoVectorDBStorage":
            from lightrag_fork.kg.nano_vector_db_impl import NanoVectorDBStorage

            return NanoVectorDBStorage
        elif storage_name == "NetworkXStorage":
            from lightrag_fork.kg.networkx_impl import NetworkXStorage

            return NetworkXStorage
        elif storage_name == "JsonDocStatusStorage":
            from lightrag_fork.kg.json_doc_status_impl import JsonDocStatusStorage
    ...
    async def apipeline_enqueue_documents(
        ...
        async with get_pipeline_enqueue_lock(self.workspace):
            # 3. Filter out already processed documents
            # Get docs ids
            ...
            if ignored_ids:
                ...
                for doc_id in ignored_ids:
                    ...
                    logger.warning(
                        f"Duplicate document detected: {doc_id} ({file_path})"
                    )
    ...
    async def apipeline_process_enqueue_documents(
        ...
        runtime_lock_ctx = get_pipeline_runtime_lock(workspace=self.workspace)
        runtime_lock_acquired = False
        try:
            await runtime_lock_ctx.__aenter__()
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status["request_pending"] = True
                log_message = (
                    "Another instance is processing the document queue. Request queued."
                )
                pipeline_status["latest_message"] = log_message
                pipeline_status["history_messages"].append(log_message)
            logger.info(log_message)
        ...
        try:
            ...
            while True:
                runtime_lock_ctx.raise_if_lost()
            ...
            log_message = f"Pipeline runtime lock lost: {e}"
            ...
            if runtime_lock_acquired:
                try:
                    await runtime_lock_ctx.__aexit__(None, None, None)
                except LockLostError:
                    raise
                except Exception as e:
                    logger.warning(f"Failed to release pipeline runtime lock: {e}")
    ...
    async def adelete_by_doc_id(
        ...
        runtime_lock_already_acquired: bool = False,
    ) -> DeletionResult:
        ...
        **Concurrency Control Design:**
        ...
        Args:
            ...
            runtime_lock_already_acquired (bool): Internal flag used by batch deletion
                flows that already hold pipeline runtime lock.
        ...
        """
        runtime_lock_ctx = None
        runtime_lock_acquired = False
        if not runtime_lock_already_acquired:
            runtime_lock_ctx = get_pipeline_runtime_lock(workspace=self.workspace)
            try:
                await runtime_lock_ctx.__aenter__()
                runtime_lock_acquired = True
                ...
                return DeletionResult(
                    status="not_allowed",
                    doc_id=doc_id,
                    message="Deletion not allowed: pipeline runtime lock is held by another instance",
                    status_code=403,
                    file_path=None,
                )
        ...
        async with pipeline_status_lock:
            if not pipeline_status.get("busy", False):
                ...
                if not job_name.startswith("deleting") or "document" not in job_name:
                    if runtime_lock_acquired and runtime_lock_ctx is not None:
                        try:
                            await runtime_lock_ctx.__aexit__(None, None, None)
                        except LockLostError:
                            raise
                            ...
                            logger.warning(
                                f"Failed to release pipeline runtime lock: {e}"
                            )
                    return DeletionResult(
                        status="not_allowed",
                        doc_id=doc_id,
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from lightrag_fork.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from lightrag_fork.utils_graph import adelete_by_relation
    ...
    async def get_entity_info(
        ...
        from lightrag_fork.utils_graph import get_entity_info
    ...
    async def get_relation_info(
        ...
        from lightrag_fork.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        """
        from lightrag_fork.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        """
        from lightrag_fork.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        """
        from lightrag_fork.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        """
        from lightrag_fork.utils_graph import acreate_relation
    ...
    async def amerge_entities(
        ...
        """
        from lightrag_fork.utils_graph import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from lightrag_fork.utils import aexport_data as utils_aexport_data
