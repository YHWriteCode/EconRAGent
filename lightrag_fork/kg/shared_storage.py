...
from lightrag_fork.exceptions import PipelineNotInitializedError
from lightrag_fork.utils import logger
from lightrag_fork.kg.lock_backend import (
    LockBackend,
    LockLease,
    LockLostError,
    LocalLockBackend,
...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    try:
        ...
        # Fallback if lightrag_fork.utils is not available
...
_lock_backend: Optional[LockBackend] = None
_lock_backend_name: str | None = None
_lock_backend_kwargs: dict[str, Any] = {}
...
def configure_lock_backend(backend: str, **kwargs):
...
def _read_optional_float_env(var_name: str, default: float | None) -> float | None:
...
def _read_optional_int_env(var_name: str, default: int | None) -> int | None:
    raw = os.getenv(var_name)
    if raw is None:
        return default
    value = raw.strip().lower()
...
def get_lock_backend() -> LockBackend:
    ...
    redis_url = _lock_backend_kwargs.get("redis_url") or os.getenv("REDIS_URI")
    if not redis_url:
        raise RuntimeError(
            "Redis lock backend requires REDIS_URI or configure_lock_backend(redis_url=...)"
        )
    ...
    key_prefix = _lock_backend_kwargs.get("key_prefix") or os.getenv(
        "LIGHTRAG_LOCK_KEY_PREFIX", "lightrag:lock"
    )
    renew_interval_s = _lock_backend_kwargs.get("renew_interval_s")
    if renew_interval_s is None:
        renew_interval_s = _read_optional_float_env(
            "LIGHTRAG_LOCK_RENEW_INTERVAL_S", None
        )
    max_retries = _lock_backend_kwargs.get("max_retries")
    if max_retries is None:
        max_retries = _read_optional_int_env("LIGHTRAG_LOCK_MAX_RETRIES", None)
...
class _BackendKeyedLockContext:
    """Backend-driven keyed lock context used by get_storage_keyed_lock."""
    ...
    def __init__(
        ...
        self._leases: list[LockLease] | None = None
        self._lease_lost_watch_task: asyncio.Task | None = None
        self._holder_task: asyncio.Task | None = None
        self._lost_error: LockLostError | None = None
        self._lost_poll_interval_s = float(
            os.getenv("LIGHTRAG_LOCK_LOST_CHECK_INTERVAL_S", "0.5")
        )
        self._owner = (
            f"{socket.gethostname()}:{os.getpid()}:{namespace}:{uuid.uuid4().hex[:8]}"
        )

    def _compose_key(self, key: str) -> str:
        return f"{self._namespace}:{key}" if self._namespace else key

    def _mark_lost_if_needed(self) -> None:
        ...
        for lease in self._leases:
            if lease.lost:
                self._lost_error = LockLostError(
                    f"Distributed lock lease lost: key={lease.key}, owner={lease.owner_id}"
                )
                return

    def raise_if_lost(self) -> None:
        self._mark_lost_if_needed()
        if self._lost_error is not None:
            raise self._lost_error

    async def _watch_lease_lost(self) -> None:
        while self._leases is not None:
            await asyncio.sleep(max(self._lost_poll_interval_s, 0.05))
            self._mark_lost_if_needed()
            if self._lost_error is None:
                continue
            holder_task = self._holder_task
    ...
    async def __aenter__(self):
        if self._leases is not None:
            raise RuntimeError("Lock context already acquired")
        self._leases = []

        for key in self._keys:
            lock_key = self._compose_key(key)
            try:
        ...
        self._lease_lost_watch_task = asyncio.create_task(self._watch_lease_lost())

        if self._enable_logging:
            logger.debug("Acquired keyed locks: namespace=%s keys=%s", self._namespace, self._keys)
        return self

    async def __aexit__(self, exc_type, exc, tb):
        if self._lease_lost_watch_task is not None:
            self._lease_lost_watch_task.cancel()
            try:
                await self._lease_lost_watch_task
            except asyncio.CancelledError:
                pass
            finally:
                self._lease_lost_watch_task = None

        if self._leases is None:
            self._holder_task = None
            self._mark_lost_if_needed()
            if self._lost_error is not None and (
                exc is None or isinstance(exc, asyncio.CancelledError)
            ):
                raise self._lost_error
            return
        for lease in reversed(self._leases):
            try:
                await self._backend.release(lease)
            except Exception as e:
                logger.warning("Failed to release lock %s: %s", lease.key, e)
        self._leases = None
        self._holder_task = None
        self._mark_lost_if_needed()
        if self._lost_error is not None and (
...
def get_storage_keyed_lock(
    ...
    lock_ttl = ttl_s
    if lock_ttl is None:
        lock_ttl = int(os.getenv("LIGHTRAG_LOCK_DEFAULT_TTL_S", "45"))

    return _BackendKeyedLockContext(
        backend=backend,
        namespace=namespace,
        keys=keys,
        ttl_s=lock_ttl,
...
def _resolve_lock_workspace(workspace: str | None = None) -> str:
    if workspace is None:
...
def get_pipeline_runtime_lock(
    workspace: str | None = None, wait_timeout_s: float | None = None
) -> _BackendKeyedLockContext:
    resolved_workspace = _resolve_lock_workspace(workspace)
    resolved_wait_timeout_s = (
        wait_timeout_s
        if wait_timeout_s is not None
        else _read_optional_float_env(
            "LIGHTRAG_PIPELINE_RUNTIME_LOCK_WAIT_TIMEOUT_S", 0.0
        )
    )
    return get_storage_keyed_lock(
        keys=["pipeline:runtime"],
        namespace=resolved_workspace,
        enable_logging=False,
        ttl_s=int(os.getenv("LIGHTRAG_PIPELINE_RUNTIME_LOCK_TTL_S", "120")),
        wait_timeout_s=resolved_wait_timeout_s,
        retry_interval_s=float(
            os.getenv("LIGHTRAG_PIPELINE_RUNTIME_LOCK_RETRY_S", "0.1")
        ),
        auto_renew=True,
    )


def get_pipeline_enqueue_lock(
    workspace: str | None = None, wait_timeout_s: float | None = None
) -> _BackendKeyedLockContext:
    resolved_workspace = _resolve_lock_workspace(workspace)
    resolved_wait_timeout_s = (
        ...
        if wait_timeout_s is not None
            ...
            "LIGHTRAG_PIPELINE_ENQUEUE_LOCK_WAIT_TIMEOUT_S", None
        )
    )
    return get_storage_keyed_lock(
        keys=["pipeline:enqueue"],
        namespace=resolved_workspace,
        enable_logging=False,
        ttl_s=int(os.getenv("LIGHTRAG_PIPELINE_ENQUEUE_LOCK_TTL_S", "60")),
        wait_timeout_s=resolved_wait_timeout_s,
        retry_interval_s=float(
            os.getenv("LIGHTRAG_PIPELINE_ENQUEUE_LOCK_RETRY_S", "0.1")
        ),
        auto_renew=True,
    )
...
class NamespaceLock:
    ...
    def __init__(
        ...
        self._ctx_var: ContextVar[Optional[Any]] = ContextVar(
