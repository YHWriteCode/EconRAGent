...
def build_skill_free_shell_planner_prompt(
    *,
    ...
    user_prompt = (
        ...
        "11. If the task needs dependency setup before the main command can run, return bootstrap_commands for that setup and explain the need in bootstrap_reason.\n"
        "12. For Python package bootstrap, prefer workspace-local commands such as python -m pip install --target ./.skill_bootstrap/site-packages <packages> instead of mutating global environments.\n"
        "13. When the Python logic is longer than a short one-liner, prefer generated_files plus an entrypoint over python -c.\n"
        "14. Do not claim success; only return the plan.\n"
        "15. If required inputs, credentials, or environment details are missing, return manual_required.\n"
