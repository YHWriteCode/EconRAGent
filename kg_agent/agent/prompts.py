...
_ROUTE_JUDGE_PROMPT_TEMPLATES: dict[str, RouteJudgePromptTemplate] = {
    "v1": RouteJudgePromptTemplate(
        ...
        planner_guidance=(
            ...
            "Use the exposed capability and skill catalogs as the first routing surface before knowledge retrieval. "
            "If a matching skill or external capability can directly satisfy the request, prefer it over KG retrieval. "
    ...
    "v2": RouteJudgePromptTemplate(
        ...
        planner_guidance=(
            ...
            "2. Treat the exposed capability catalog and available_skills as the first routing surface; only fall back to KG retrieval when no matching skill or capability can directly satisfy the request.\n"
            "3. Use memory tools only for contextual follow-ups.\n"
            "4. Use web_search for direct URLs, realtime/freshness needs, or correction-driven refresh.\n"
            "5. Preserve tool order unless there is a concrete reason to reorder it.\n"
            "6. Use the capability catalog to evaluate planner-visible external capabilities from their descriptions, tags, executor, and input contract.\n"
            "7. Use available_skills to evaluate local or VM-hosted skills independently from the capability plane.\n"
            "8. Prefer skill_plan when a matching skill is a more concrete fit than native KG/web/memory tools.\n"
            "9. Prefer a matching external capability when it is a more concrete fit than native tools and no better skill match exists.\n"
            "10. When emitting skill_plan, include structured constraints when they are explicit in the user query, such as file paths, output paths, mode or format hints, dry-run intent, operation hints, CLI-style args, or an explicit shell_mode request.\n"
            "11. Only provide tool args that fit the selected capability's input contract; omit framework-reserved fields unless the tool contract explicitly needs them.\n"
            "12. Never invent capabilities or skills outside the provided planner surfaces.\n"
        ),
    ),
}
...
def build_skill_free_shell_planner_prompt(
    *,
    ...
    user_prompt = (
        ...
        "3. If the docs provide Python examples, especially many examples, you may convert or combine them into generated_files and then execute a generated entrypoint.\n"
        "4. Generated files must stay under a relative workspace path such as .skill_generated/.\n"
        "5. When you return generated_files, you may omit command and instead set entrypoint plus optional cli_args. The runtime can materialize the execution command from that entrypoint.\n"
        "6. If multiple generated files are needed, make one generated file the clear executable entrypoint and put its path in entrypoint.\n"
        "7. Match the command to the declared runtime target instead of the host machine you are running on.\n"
        "8. The command may use pipelines, chaining, and shell features when justified by the task.\n"
        "9. Respect runtime_target.network_allowed=false by avoiding downloads, package installs, or external network calls unless the task explicitly depends on network access and you therefore return manual_required.\n"
        "10. Respect runtime_target.supports_python=false by avoiding generated Python scripts in that case.\n"
        "11. Treat natural-language dependency guidance anywhere in SKILL.md as valid planning input, even when there is no explicit Dependencies section. If SKILL.md says a package, CLI, runtime, library, or setup step is required, you may plan bootstrap_commands from that guidance.\n"
