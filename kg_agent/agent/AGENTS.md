...
- When uploaded chat attachments are present, keep them as a first-class planner input instead of only burying them in the free-form query text. Generic “this file / attachment” requests should prefer attachment-context answering unless the user clearly asks for a file-format-specific operation.
- If a local skill is still the right route for an uploaded attachment and the user did not type an explicit filesystem path, it is acceptable to auto-bind the uploaded file path into common input constraint keys before skill execution.
