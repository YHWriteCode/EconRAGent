...
- The image now bakes in the core financial research stack during `docker build`: `numpy`, `pandas`, `yfinance`, `akshare`, `linearmodels`, `statsmodels`, and `backtrader`. Shell skills like `financial-researching` should therefore avoid the most expensive per-run bootstrap path for stock-data fetch, panel modeling, and local backtests; workspace-local bootstrap is still reserved for dependencies that are not already present in the image.
- `skills/financial-researching/scripts/prepare_panel_data.py` must use direct `numpy` calls rather than deprecated `pd.np`; the current runtime image carries `numpy`, so panel-factor preparation should stay compatible with modern `pandas` releases inside the dockerized shell runtime.
- `skills/financial-researching/scripts/fetch_model_backtest.py` now accepts `--output`, retries transient AKShare fetch failures, and falls back to signal-only / momentum-style single-asset execution when the fetched panel collapses to fewer than 2 stocks; this keeps dockerized shell runs from dying on `--output` argparse errors or `PanelOLS` zero-division when only one ticker is available.
- Shell-runtime command materialization now prefers the mirrored writable workspace copy of skill entrypoints, and explicit repair commands that referenced `/app/skills/<skill>` are rewritten onto the runtime workspace before execution. This prevents free-shell repair loops from `cd`-ing back into the read-only image layer and failing to create `output/`.
- `skills/financial-researching/scripts/fetch_model_backtest.py` now uses a target-aware market-data fallback chain (`AKShare -> yfinance`) during the initial fetch step. If the requested backtest target still cannot be fetched after both sources and retries, the script exits immediately instead of continuing with a panel built from unrelated tickers.
- The skill runtime now mirrors `loaded_skill.file_inventory` into each runtime workspace before execution. This preserves relative paths such as `scripts/...`, `references/...`, `assets/...`, and `SKILL.md` for free-shell generated scripts that execute from the workspace rather than directly from the immutable skill source tree.
