...
- Chat retrieval also treats `workspace=all` as an aggregation sentinel when dynamic workspaces are enabled. The app wires the workspace registry into `AgentCore` so KG/graph retrieval tools fan out across registered workspaces instead of falling back to the default workspace.
- App bootstrap should register both `KG_AGENT_DEFAULT_WORKSPACE` and `KG_AGENT_NETWORK_INGEST_WORKSPACE` when configured. The latter is also passed to the scheduler so unscoped crawler sources and Discover filtering resolve to the same dedicated network-ingest knowledge base.
- Scheduler startup can bootstrap monitored sources from `KG_AGENT_SCHEDULER_BOOTSTRAP_SOURCES_JSON` or `KG_AGENT_SCHEDULER_BOOTSTRAP_SOURCES_FILE`; load and upsert those sources before calling `scheduler.start()` so Discover has recurring feed inputs after service restart.
...
- Discover event routes are backed by crawler scheduler state. Prefer event clusters when present, but also expose recent feed items from `CrawlStateRecord.recent_item_keys` so the WebUI news stream is not empty when clustering has not produced records yet. Event cards should always return a non-empty `headline`, falling back through document metadata, summary text, and URL-derived labels when crawler clustering has no stored headline.
